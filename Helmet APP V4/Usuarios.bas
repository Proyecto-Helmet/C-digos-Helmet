...
Public Sub chequearMotoID (motoid As String) As Boolean   'chequea si ya existe un motoID registrado con otro usuario, un vehículo solo puede tener un casco principal
	'devuelve false si no hay un motoid registrado y true en caso contrario

	Log("--------------------------------------------")
	Log("Chequeando en la base de datos si el motoID ya se encuentra registrado a nombre de otro usuario...")
	Dim motoidINT As Int
	motoidINT=motoid'PASO A INTEGER
	Dim cursor As Cursor
	Dim flag As Boolean
	flag=False
	'Dim datosString As String
	...
	For i=0 To cursor.RowCount-1
		...
		datos.motoID=cursor.GetInt("motoid")
		'Chequea Si el motoid ingresado es igual a un motoid en la base de datos
		If datos.motoID=motoidINT Then
			
			Log("Ya hay un motoID registrado con este usuario!")
			Log("El DNI es: "& datos.dni)
			flag=True
			
		Else
			Log("Ningún motoID registrado con el usuario ingresado")
			flag=False
		End If
	Next
...
Public Sub insertarMotoID (motoidIngresado As String, dniIngresado As String)
	
	Dim MotoIDIngresadoLONG As Long
	Dim DniIngresadoINT As Int
	
	MotoIDIngresadoLONG=motoidIngresado'paso de string a long
	DniIngresadoINT=dniIngresado'paso de string a int
	
	Log("--------------------------------------------")
	Log("ingresando MotoID: "& MotoIDIngresadoLONG&" en la base de datos, bajo el DNI: "& dniIngresado)
	Sql.ExecNonQuery2("UPDATE usuarios SET motoid=? WHERE dni=?" , Array As Object (MotoIDIngresadoLONG,dniIngresado))
	Log ("Moto ID ingresado")
End Sub
